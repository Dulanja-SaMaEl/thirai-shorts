            ...
            BEGIN
                ...
                ELSE
                    ...
                    VALUES 
                        ...
                        (director_uuid, director_uuid, json_build_object('sub', director_uuid::text, 'email', 'director@thiraiplus.com')::jsonb, 'email', NOW(), NOW(), NOW()),
                        (viewer_uuid, viewer_uuid, json_build_object('sub', viewer_uuid::text, 'email', 'viewer@thiraiplus.com')::jsonb, 'email', NOW(), NOW(), NOW())
                    ON CONFLICT DO NOTHING;
                END IF;
...
-- 8. Seed Official Festival Awards (22 Award Categories Sample Allocations)
CREATE TABLE IF NOT EXISTS public.festival_awards (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
